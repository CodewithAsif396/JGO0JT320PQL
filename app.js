...
async function renderActivePositions() {
    ...
    try {
        ...
        c.innerHTML = '<div style="padding:0 12px 12px;">' + pending.map(function (t) {
            ...
            var date = new Date(t.createdAt).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Karachi' });
...
function addLocalPosition(sym, dir, amount) {
    ...
    var pos = { id: Date.now(), sym: sym, dir: dir, amount: amount, entry: entry, time: new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Karachi' }), status: 'active', durationMs: durationMs };
...
function initSocket() {
    ...
    socket.on('new_signal', (signal) => {
        ...
        const sigTime = signal.entryTime ? new Date(signal.entryTime).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Karachi' }) : '';
...
async function loadTradeHistory() {
    ...
    try {
        ...
        const rows = trades.map(t => {
            ...
            const date = new Date(t.createdAt).toLocaleString('en-US', { timeZone: 'Asia/Karachi' });
